...
export default function LeftPanel() {
  ...
  return (
    <div className="container  h-full px-4 w-full mx-auto sm:px-6 md:px-0 py-1">
      <div className="border-b sticky top-0 border-ink/10 flex justify-center px-6 sm:px-5 mx-auto w-full py-9">
        <div className="flex flex-col justify-start">
          <div className="flex gap-2 place-content-between">
            <Avatar
              src={userData?.user.user_metadata.avatar_url}
              className="ring-2 ring-aura-violet/30 ring-offset-2 ring-offset-parchment-100"
            />
          </div>
        </div>
      </div>
    ...
    <Link href={`/${name.toLowerCase()}`}>
      <button
        className={`flex gap-2 items-center p-4 justify-center w-full rounded-xl transition-colors ${
          isSelected
            ? "bg-parchment-200 text-ink shadow-[inset_0_0_0_1px_rgba(33,27,18,0.08)]"
