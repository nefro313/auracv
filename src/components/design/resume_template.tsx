...
/* Shared section heading — serif title with a hairline underline. */
const SectionTitle: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => (
  <h2 className="mb-3 border-b border-ink/15 pb-1.5 font-fraunces text-lg font-medium tracking-tight text-ink">
    {children}
  </h2>
);

const ResumeTemplate: React.FC<ResumeTemplateProps> = ({ profile }) => {
  // "Has experience" is true when the user has any work entries. When false the
  // Experience section is hidden and Projects is shown ahead of it.
  const hasExperience =
  ...
  const experienceSection = hasExperience && (
    <section className="mb-6">
      ...
      {profile.work.map((work, index) => (
        <div key={`workResume-${index}`} className="mb-4">
    ...
    profile.projects.projects[0].title !== "" && (
      <section className="mb-6">
        ...
        {profile.projects.projects.map((project, index) => (
          <div key={`projectResume-${index}`} className="mb-4">
            <h3 className="text-base font-semibold text-ink">
              {project.title}
            </h3>
    ...
    profile.education[0].institution !== "" && (
      <section className="mb-6">
        ...
        {profile.education.map((edu, index) => (
          <div key={`educationResume-${index}`} className="mb-4">
            <div className="flex flex-wrap items-baseline justify-between gap-x-3">
              <h3 className="text-base font-semibold text-ink">
                {edu.institution}
              </h3>
    ...
    profile.certificates[0].name !== "" && (
      <section className="mb-6">
        ...
        {profile.certificates.map((cert, index) => (
          <div key={`certificateResume-${index}`} className="mb-3">
    ...
    profile.awards[0].title !== "" && (
      <section className="mb-6">
        ...
        {profile.awards.map((award, index) => (
          <div key={`awardResume-${index}`} className="mb-3">
            <h3 className="text-base font-semibold text-ink">{award.title}</h3>
            {award.awarder && (
              <p className="text-sm">Awarder: {award.awarder}</p>
            )}
      ...
      <section>
        ...
        {profile.references.map((ref, index) => (
          <blockquote
            ...
            className="mb-3 border-l-2 border-ink/15 pl-4 italic"
          >
  ...
  return (
    <div className="min-h-svh bg-parchment-200/60 py-8 font-outfit sm:py-14">
      <div
        id="resume-template"
        className="mx-auto max-w-[820px] rounded-2xl border border-ink/10 bg-white p-8 leading-relaxed text-ink-soft shadow-[0_24px_60px_-32px_rgba(33,27,18,0.35)] sm:p-12"
      >
        <header className="mb-6 text-center">
          ...
          <div className="mt-3 flex flex-wrap items-center justify-center gap-x-5 gap-y-1.5 text-sm text-ink-soft">
